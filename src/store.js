  ...
  generateNarration: async (userPrompt = '', mode = 'refine', seasonContext = '') => {
    ...
    try {
      ...
      if (mode === 'photo' && photos.length > 0) {
        ...
        prompt = `당신은 한국 농업 전문 숏폼 영상 나레이션 작가입니다.
아래 사진들을 분석하고, 농장 정보를 바탕으로 60초 이내의 숏폼 영상 나레이션을 작성해주세요.
...
요구사항:
- 사진 속 장면을 구체적으로 묘사해주세요${seasonContext ? '\n- 계절감과 날씨 분위기를 나레이션에 자연스럽게 녹여주세요' : ''}
- 따뜻하고 정감 있는 한국어 구어체
- 농부의 자부심과 정성이 느껴지는 톤
- 시청자에게 신뢰감을 주는 내용
...
요구사항:
- 따뜻하고 정감 있는 한국어 구어체${seasonContext ? '\n- 계절감과 날씨 분위기를 나레이션에 자연스럽게 녹여주세요' : ''}
- 농부의 자부심과 정성이 느껴지는 톤
- 시청자에게 신뢰감을 주는 내용
